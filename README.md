...
Сборка полностью укомплектована и готова к игре как для одиночной игры с ботами (Steam / Non-Steam), так и для выделенного сервера (HLDS / ReHLDS):

1. **Скачайте архив:**
   * Вверху этой страницы нажмите зеленую кнопку **`Code`** ➔ **`Download ZIP`**.
2. **Распакуйте в папку `cstrike`:**
   * Перенесите все папки из архива (`addons`, `models`, `sound`, `sprites`, `gfx` и др.) прямо в папку вашей игры `cstrike/` с подтверждением слияния и замены:
     * 📁 **Steam:** `Steam/steamapps/common/Half-Life/cstrike/`
     * 📁 **Non-Steam:** `ваша папка с игрой/cstrike/`
3. **Запустите игру:**
   * Зайдите в **«Новая игра»** или напишите в консоли `map de_dust2`. Мод, CSDM, боты YaPB и способности активируются сразу!
